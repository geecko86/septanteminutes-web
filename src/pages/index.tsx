...
export default function Home() {
    ...
    const { setPlaying, isPlaying, setPlaybackMP3, setPlaybackNum, setPlaybackTitle, playbackTitle } = usePlayback();
    ...
    return (
        <div ref={subroot} className={styles.home_subroot} key={"home_subroot"}>
            <div ref={root} className={styles.home_root} key={"home_root"}>
                <motion.div className={styles.home} key={"home"} ref={home} tabIndex={0} onKeyDown={handleKeysDown} onKeyUp={handleArrowsUp} style={{ translateX: newScrollX }}>
                    <Head>
                            ...
                            [...seasons].reverse().map((season, i) => (
                                <Season key={`${season.name}_visible1`} seasonTitle={`SAISON ${season.name}`} chair={Chairs[i % 4]} className={styles.season_frame}>
                                    {season.episodes.toReversed().map((ep: episode) => (
                                        <HomeAlbum id={`art_${ep.num}`} guest={ep.title} key={`${ep.num}_visible1`} image={ep.img} num={ep.num} onClick={() => {
                                            setPlaybackMP3(ep.mp3);
                                            setPlaybackNum(Number(ep.num));
                                            setPlaybackTitle(ep.title);
                                        }} />
                                    ))}
                                </Season>
                            ))
